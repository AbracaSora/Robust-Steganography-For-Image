...
class ImageReconstructionLoss(torch.nn.Module):
    ...
    def forward(self, x, x_recon, global_step, secret=None, secret_recon=None):
        ...
        perceptual = torch.clamp(self.perceptual_loss(x_recon_input, x_input), min=0).mean()
        ...
        image_loss = image_loss / torch.exp(self.logvar) + self.logvar

        secret_loss = 0.0
        l1 = 0.0
        ssim = 0.0
        if secret is not None and secret_recon is not None:
            l1, ssim = self.compute_secret_loss(secret, secret_recon)
            secret_loss = (l1 + ssim) * self.secret_weight
        ...
        total_loss = (image_loss * image_weight + secret_loss * self.secret_weight) / (
                    image_weight + self.secret_weight)

        if perceptual < 0:
            print(f"perceptual:{perceptual}, perceptual_loss(x_recon, x):{self.perceptual_loss(x_recon, x)}")
            diff = torch.abs(x - x_recon).mean()
